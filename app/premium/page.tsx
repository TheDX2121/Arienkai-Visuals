    ...
    <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="mx-auto mb-10 max-w-3xl text-center">
        ...
        <h1 className="text-5xl font-black tracking-tight">
          High-level resources without luxury pricing.
        </h1>

        <p className="mt-4 text-white/55">
          The subscription layer is designed for affordability first. Stripe or Razorpay can be connected in the next phase.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {tiers.map((tier) => (
          <article key={tier.name} className={`glass-panel rounded-[2rem] p-6 ${tier.featured ? "border-brand/60 shadow-glow" : ""}`}>
            ...
            <Link href={tier.href} className={tier.featured ? "primary-button mt-8 w-full justify-center" : "secondary-button mt-8 w-full justify-center"}>
              Choose {tier.name}
            </Link>
          </article>
        ))}
