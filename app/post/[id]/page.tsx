...
export default async function PostDetailPage({ params }: PostProps) {
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1.2fr_.8fr] lg:px-8">
      <div
        className={`min-h-[560px] rounded-[2rem] bg-gradient-to-br ${post.gradient} p-6 shadow-card`}
      >
      ...
      <aside className="glass-panel h-fit rounded-[2rem] p-6">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          {post.animeTags.map((tag) => (
            <span className="pill bg-brand/10 text-red-100" key={tag}>
              {tag}
            </span>
          ))}
        </div>
        ...
        <div className="mt-7 rounded-2xl bg-black/25 p-4">
          ...
          <p className="mt-2 text-sm text-white/50">
            Users can score composition, color, title readability, motion, and click appeal.
          </p>
          ...
          <input
            ...
            min="1"
            max="10"
            defaultValue={Math.round(post.rating)}
